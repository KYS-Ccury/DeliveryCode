...
void CustomerHandler::handleWriteReview(Session* session, const std::string& body) {
    try {
        ...
        if (!uid || !orderID || content.empty()) {
            sendError(session, CmdCustomer::REQ_WRITE_REVIEW, Status::BAD_REQUEST, "필수 항목 누락");
            return;
        }
        ...
        if (chk.empty()) { sendError(session, CmdCustomer::REQ_WRITE_REVIEW, Status::FORBIDDEN, "배달 완료된 주문만 리뷰 가능"); return; }
