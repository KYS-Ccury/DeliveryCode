...
void AdminHandler::handleAdminLogin(Session* session, const std::string& body) {
    try {
        ...
        std::string id = req.value("login_id", "");
        std::string pw = req.value("password",  "");

        auto rows = db.executeQuery(
            "SELECT user_id, name FROM users "
            "WHERE login_id='" + esc(id) + "' AND password='" + esc(pw) + "' "
        ...
        std::cout << "[Admin] 로그인: " << rows[0].at("name") << "\n";
...
void AdminHandler::handleForceCancel(Session* session, const std::string& body) {
    try {
        ...
        if (!custRows.empty() && EpollServer::s_instance) {
            ...
            if (cs)
                CustomerHandler::pushOrderStatus(cs, orderID, 0, "관리자에 의해 주문이 취소되었습니다: " + reason);
        }
